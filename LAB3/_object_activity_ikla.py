...
def enroll_to_subject(student, subject):
    if not isinstance(student, Student) or not isinstance(subject, Subject):
        return "Error"
    if not search_enrollment_subject_student(subject, student):
        enrollment_list.append(EnrolledSubject(student, subject))
        return "Done"
    else:
...
def search_student_enroll_in_subject(subject):
    enrollment_in_subject = []
    for enrollment in enrollment_list:
        if enrollment.subject == subject:
            enrollment_in_subject.append(enrollment)
    return enrollment_in_subject
...
def search_subject_that_student_enrolled(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            student_subjects.append(enrollment)
...
def assign_grade(student, subject, grade):
    if not isinstance(student, Student) or not isinstance(subject, Subject) or grade not in grade_list:
        return "Error"
...
def get_teacher_teach(subject_search):
    for enrollment in enrollment_list:
        if enrollment.subject == subject_search:
...
def get_student_record(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            enrolled_subject = enrollment.subject
            student_record[enrolled_subject.subject_id] = [enrolled_subject.subject_name, enrollment.grade]
...
def get_student_GPS(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            total_grades += grade_to_count(enrollment.grade) * enrollment.subject.credit
            total_hours += enrollment.subject.credit
    return total_grades / total_hours
...
# NOTE : Test Case #1 : test enroll_to_subject complete ###
...
# NOTE : Test case #2 : test enroll_to_subject in case of invalid argument
print("Test case #2 : test enroll_to_subject in case of invalid argument")
print("Answer : Error")
print(enroll_to_subject('66010001','CS101'))
print("")

# NOTE : Test case #3 : test enroll_to_subject in case of duplicate enrolled
...
# NOTE : Test case #7 : test search_student_enrolled_in_subject
...
# NOTE : Test case #8 : get_no_of_student_enrolled
print("Test case #8 get_no_of_student_enrolled")
print("Answer : 5")
print(get_no_of_student_enrolled(subject_list[0]))
print("")

# NOTE : Test case #9 : search_subject_that_student_enrolled
...
print(get_student_record(student_list[3]))
print("")

# NOTE : Test case #14 : get_student_GPS
print("Test case #14 get_student_GPS")
print("Answer : 3.0")
print(get_student_GPS(student_list[3]))