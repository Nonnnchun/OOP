...
def search_student_enroll_in_subject(subject):
    ...
    result = [enrollment for enrollment in enrollment_list if enrollment.subject == subject]
    return result
...
def search_subject_that_student_enrolled(student):
    ...
    result = [enrollment for enrollment in enrollment_list if enrollment.student == student]
    return result
...
def get_student_record(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            record[enrollment.subject.subject_id] = [enrollment.subject.subject_name, enrollment.grade, ]
    return record
...
def get_student_GPS(student):
    ...
    for subject_id, details in record.items():
        grade = details[1]
        if grade in grade_mapping:
            total_points += grade_mapping[grade] * search_subject_by_id(subject_id).credit
            total_credits += search_subject_by_id(subject_id).credit
